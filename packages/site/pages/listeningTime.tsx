...
const msToHoursMinutesSeconds = (ms: number) => {
  ...
  return `${hours} hour${hours > 1 || hours == 0 ? 's' : ''}, ${minutes} minute${minutes > 1 || minutes == 0 ? 's' : ''}, ${seconds} second${seconds > 1 || seconds == 0 ? 's' : ''}`
}
...
const InsightsComponent = ({ data }) => (
  <>
    ...
    Monthly Average: { msToHoursMinutesSeconds(data.monthly_average) } <br /> <br />
...
const ListeningTime = () => {
  ...
  return (
    <>
      ...
      <LoadedComponent data={insights.data} error={insights.error} component={InsightsComponent} />
