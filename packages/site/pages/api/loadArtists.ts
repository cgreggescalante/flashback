...
export default async (req, res) => {
  ...
  try {
    artist_ids = (await connection.execute("SELECT DISTINCT ARTIST_ID FROM TRACK")).rows;
    ...
    while (i < artist_ids.length) {
      console.log(i)
      await spotifyApi.getArtists(artist_ids.slice(i, i + 50))
    ...
    const sql = "INSERT INTO ARTIST (ID, NAME, FOLLOWERS, POPULARITY) VALUES (:id, :name, :followers, :popularity)"
